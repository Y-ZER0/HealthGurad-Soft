...
export const mockMedications: Medication[] = [
  ...
  },
  {
    ...
    Instructions:
      "Regular INR monitoring required. Avoid vitamin K rich foods.",
    IsActive: true,
    PrescribedBy: 2,
      ...
      medication.TimeOfDay.forEach((time) => {
        ...
        if (isOverdue) {
          if (random < 0.85) {
            ...
            takenDate.setMinutes(
              takenDate.getMinutes() + Math.floor(Math.random() * 30)
            );
...
export const mockMessages: Message[] = [
  {
    ...
    MessageText:
      "Hi Robert, I've reviewed your latest blood pressure readings. They're looking much better! Keep up with your medication schedule.",
  ...
  },
  {
    ...
    MessageText:
      "Thank you, Doctor! I've been very consistent with taking my medications. Should I continue with the same dosage?",
    SentAt: new Date(
  ...
  },
  {
    ...
    RecipientID: 1, // Robert Williams patient
    MessageText:
      "Yes, continue with your current dosage. Let's schedule a follow-up in two weeks.",
    SentAt: new Date(
  ...
  },
  {
    ...
    MessageText:
      "James, I noticed your heart rate has been irregular. Please come in for an EKG this week if possible.",
  ...
  },
  {
    ...
    MessageText:
      "Patricia, your glucose levels are trending higher. Let's discuss adjusting your diet and medication during our next appointment.",
...
export const mockAlertThresholds: AlertThreshold[] = [
  // Patient 1 - Robert Williams
  {
    ...
    MaxValue: 140,
    SetBy: 1,
    SetAt: new Date("2024-01-15").toISOString(),
    IsActive: true,
  },
  {
    ThresholdID: 2,
  ...
  // Patient 2 - Mary Thompson
  {
    ...
    MaxValue: 140,
    SetBy: 1,
    SetAt: new Date("2024-03-15").toISOString(),
    IsActive: true,
  },
  {
    ThresholdID: 6,
  ...
  // Patient 3 - James Anderson (custom thresholds by cardiologist)
  {
    ...
    MaxValue: 135,
    SetBy: 2,
    SetAt: new Date("2024-02-10").toISOString(),
    IsActive: true,
  },
  {
    ThresholdID: 10,
  ...
  // Patient 4 - Patricia Davis (diabetic - different glucose thresholds)
  {
    ...
    MaxValue: 140,
    SetBy: 3,
    SetAt: new Date("2024-06-05").toISOString(),
    IsActive: true,
  },
  {
    ThresholdID: 14,
  ...
  // Patient 5 - John Martinez
  {
    ...
    MaxValue: 140,
    SetBy: 1,
    SetAt: new Date("2024-09-12").toISOString(),
    IsActive: true,
  },
  {
    ThresholdID: 18,
...
export const getPatientById = (id: number) =>
  mockPatients.find((p) => p.PatientID === id);
export const getDoctorById = (id: number) =>
  mockDoctors.find((d) => d.DoctorID === id);
export const getVitalRecordsByPatientId = (patientId: number) =>
  mockVitalRecords.filter((v) => v.PatientID === patientId);
export const getMedicationsByPatientId = (patientId: number) =>
  mockMedications.filter((m) => m.PatientID === patientId);
export const getAlertsByPatientId = (patientId: number) =>
  mockAlerts.filter((a) => a.PatientID === patientId);
export const getAlertThresholdsByPatientId = (patientId: number) =>
  mockAlertThresholds.filter((t) => t.PatientID === patientId);
export const getActiveAlerts = () =>
  mockAlerts.filter((a) => a.Status === "Active");
export const getPatientsByDoctorId = (doctorId: number) =>
  mockPatients.filter((p) => p.AssignedDoctorID === doctorId);
export const getMedicationLogsByPatientId = (patientId: number) =>
  mockMedicationLogs.filter((log) => log.PatientID === patientId);
export const getMedicationLogsByMedicationId = (medicationId: number) =>
  mockMedicationLogs.filter((log) => log.MedicationID === medicationId);
export const getMessagesByUserId = (userId: number) =>
  mockMessages.filter((m) => m.SenderID === userId || m.RecipientID === userId);
export const getUnreadMessagesByRecipientId = (recipientId: number) =>
  mockMessages.filter((m) => m.RecipientID === recipientId && !m.IsRead);
